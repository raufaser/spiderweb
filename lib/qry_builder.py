...
def query_build(logger,parameters,band_frequencies,modes_frequencies,continents_cq,enable_cq_filter):
    ...
    try:
        ...
        query_string = '''SELECT rowid, spotter AS de, freq, spotcall AS dx, time, spotdxcc, spot_additional.program, spot_additional.source, spot_additional.reference, spot_additional.clean_comment as comm
               FROM spot INNER JOIN spot_additional ON spot.rowid = spot_additional.spot_id 
               WHERE rowid > ''' + (str(last_rowid))
